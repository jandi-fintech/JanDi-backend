"""
routes_with_swagger_comments.py ― JanDi 금융 데이터 API 라우터
================================================================
본 모듈은 JanDi 프로젝트에서 사용하는 REST/웹소켓 엔드포인트를 정의합니다.
Swagger(OpenAPI) 문서에 **친절하게** 노출되도록 각 엔드포인트∙모델에 상세
설명을 추가했습니다.

❐ 제공 기능
-----------
1. **거래 내역** (CODEF)
2. **국내 주식·지수 시세** (한국투자증권)
3. **해외 주식·지수 시세**
4. **실시간 가격 WebSocket 스트림**

모든 외부 API 호출은 I/O 바운드이므로 `asyncio.run_in_executor` 로 넘겨
비동기로 처리합니다.
"""
